...
public class ZeldaTreasureRoom extends TiledImageRoom 
{
	private static Image WALL_TEST, DUNGEON_FLOOR;

	public ZeldaTreasureRoom(GameWorld gameWorld)	{	this(gameWorld, 0, 0);	}

	public ZeldaTreasureRoom(GameWorld gameWorld, int row, int col) 
	{
		super(gameWorld, row, col);
		setRoomType(RoomType.ZELDA_ROOM_TREASURE);
		...
		try {
			getItemList().add(new HeartContainerItem(getGameWorld()));
		getItemList().add(new PegasusBootsItem(getGameWorld()));
		getItemList().add(new RedPotionItem(getGameWorld()));
		getItemList().add(new WhiteSwordItem(getGameWorld()));
			} catch (SlickException e) {
	...
	public int getWidthInTiles() 										{	return getaStarMapWidth();			}

	public int getHeightInTiles() 										{	return getaStarMapHeight();			}

	public void pathFinderVisited(int x, int y) 						{										}

	public boolean blocked(PathFindingContext context, int tx, int ty) 	{	return getaStarMap()[ty][tx] != 0;	}

	public float getCost(PathFindingContext context, int tx, int ty) 	{	return 1.0f;						}
}
